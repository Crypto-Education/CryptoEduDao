...
it("ELIGIBILITY", async () => {
  ...
  await truffleAssert.reverts(batchMDeployed.createAppendBatch("Batch 1 |Test ido ", false, {from: accounts[1]}));
  ...
  await assert.equal(idoCreated1.isEligible({from : accounts[3]}),true, "Account not eligible");
  await assert.equal(idoCreated2.isEligible({from : accounts[0]}),true, "Account not eligible");
  await assert.equal(idoCreated2.isEligible({from : accounts[9]}),true, "Account not eligible");
  await assert.equal(idoCreated3.isEligible({from : accounts[0]}),true, "Account not eligible");
  await assert.equal(idoCreated3.isEligible({from : accounts[7]}),true, "Account not eligible");
  await assert.equal(idoCreated4.isEligible({from : accounts[1]}),true, "Account not eligible");
  await assert.equal(idoCreated4.isEligible({from : accounts[8]}),true, "Account not eligible");
...
it("DEPOSIT", async () => {
  ...
  //tous les comptes sont éligibles pour voter
  assert.isFalse(await cDAOAdmins1.checkEligibility.call(accounts[3]));
  await truffleAssert.reverts(batchCreated1.createAppendBatch("Batch 1 |Test ido ", false, {from: accounts[1]}));
...
it("SET TOKEN", async () => {
  ...
  //tous les comptes sont éligibles pour voter
  assert.isFalse(await cDAOAdmins1.checkEligibility.call(accounts[3]));
  await truffleAssert.reverts(batchCreated1.createAppendBatch("Batch 1 |Test ido ", false, {from: accounts[0]}));
 ...
 // await truffleAssert.reverts(ballotCreated1.vote( proposal, {from : accounts[7]}), "Amount deposited in capital is not enough or not having all deposited Ceca in your wallet"); // cant vote because deposited balance < 100
  ...
  await truffleAssert.reverts(idoCreated1.setIdoToken(accounts[1], web3.utils.toWei("450"),batchCreated1.address, {from: accounts[0]})); 
