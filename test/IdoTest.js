...
it("DEPOSIT", async () => {
  ...
  assert.ok(idoCreated1.depositForIdo(web3.utils.toWei("7"), fusd.address, {from: accounts[1]})); // manque 7
...
it("SET TOKEN", async () => {
  ...
  //doit etre true car dans le it precedent on a envoye 350
  assert.isTrue(await cDAOAdmins1.checkEligibility.call(accounts[3]));
  //await truffleAssert.reverts(batchManagerDeployed.createAppendBatch("Batch 1 |Test ido ", false, {from: accounts[0]}));
